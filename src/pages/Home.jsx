...
import { cleanupNostrService, initNostrService } from '../services/nostr';
    ...
    <div className="h-dvh bg-base-100 flex flex-col overflow-hidden relative">
      ...
      <div className="flex-1 overflow-hidden relative bg-base-100">
        <div className="h-full flex flex-col md:flex-row overflow-hidden">
          <div
            className={`flex-1 flex flex-col justify-center items-center p-4 pb-24 md:p-4 overflow-y-auto ${activeTab !== 'dialer' ? 'hidden md:flex' : ''}`}
          >
            <Dialer />
          </div>
          ...
          <div
            className={`md:hidden flex-1 overflow-hidden ${activeTab === 'history' ? 'flex' : 'hidden'}`}
          >
            <CallHistory onViewContact={handleViewContact} />
          </div>
          <div
            className={`md:hidden flex-1 overflow-hidden ${activeTab === 'contacts' ? 'flex' : 'hidden'}`}
          >
      ...
      <div className="md:hidden absolute bottom-0 left-0 right-0 pointer-events-none z-50">
        <div className="mx-4 mb-4 rounded-3xl bg-base-100/95 backdrop-blur-xl border border-base-200/80 pb-safe shadow-[0_18px_40px_-18px_rgba(0,0,0,0.6)] relative pointer-events-auto">
          <div className="grid grid-cols-3 items-center gap-2 p-2">
            ...
            >
              <span
                className={`flex h-9 w-9 items-center justify-center rounded-full ${activeTab === 'dialer' ? 'bg-primary-content/15' : 'bg-base-100/70'}`}
              >
            ...
            >
              <span
                className={`flex h-9 w-9 items-center justify-center rounded-full ${activeTab === 'history' ? 'bg-primary-content/15' : 'bg-base-100/70'}`}
              >
            ...
            >
              <span
                className={`flex h-9 w-9 items-center justify-center rounded-full ${activeTab === 'contacts' ? 'bg-primary-content/15' : 'bg-base-100/70'}`}
              >
