...
export interface WheelPickerProps {
  ...
  itemHeight?: number;
  disabled?: boolean;
  /** Display-only: no input, full opacity (unlike disabled). */
  readOnly?: boolean;
...
function optionValue(option: WheelPickerOption | undefined) {
  if (!option) {
    return "";
  }
  return typeof option === "string" ? option : option.value;
}
function optionLabel(option: WheelPickerOption | undefined) {
  if (!option) {
    return "";
  }
...
export function WheelPicker({
  ...
  const { height } = useMemo(() => {
    const rows = Math.max(
      3,
      visibleCount % 2 === 0 ? visibleCount + 1 : visibleCount,
    );
    return { height: rows * itemHeight };
  ...
  const paint = useCallback(
    (s: number) => {
      ...
      if (drum) {
        drum.style.transform = `translateY(${-s * itemHeight}px)`;
      }
    },
    [itemHeight],
  );
  ...
  const onPointerDown = useCallback(
    (event: PointerEvent<HTMLDivElement>) => {
      if (locked || reduce || event.pointerType === "touch") return;
      event.currentTarget.setPointerCapture(event.pointerId);
      beginDrag(event.clientY);
    },
    [locked, reduce, beginDrag],
  );
  ...
  const onKeyDown = useCallback(
    ...
    [locked, sound, last, step, getPlayer],
  );
  ...
  }, [reduce, locked, beginDrag, moveDrag, endDrag, onWheel]);
  ...
  return (
    ...
    >
      <div
        className="pointer-events-none absolute inset-x-0 top-1/2 z-10 -translate-y-1/2 border-y border-white/12 bg-white/[0.04]"
        style={{ height: itemHeight }}
      />
      <ul
        ref={drumRef}
        aria-hidden
        className="relative m-0 list-none p-0 will-change-transform"
        style={{ paddingTop: (height - itemHeight) / 2 }}
      >
