...
import { FaDeleteLeft, FaPhone } from 'react-icons/fa6';
  ...
  return (
    <div className="flex flex-col h-full w-full max-w-sm mx-auto pb-8">
      <div className="flex-1 flex flex-col justify-end items-center mb-8 px-4">
        {target.length === 0 ? (
          <span className="text-sm opacity-50 mt-2">Enter number</span>
        ) : (
          <input
            type="text"
            readOnly
      ...
      <div className="grid grid-cols-3 gap-4 px-8 mb-8">
        {keys.map((key) => (
          ...
          >
            ...
            {key.sub && (
              <span className="text-[10px] opacity-50 font-bold tracking-widest">
                {key.sub}
              </span>
            )}
          </button>
