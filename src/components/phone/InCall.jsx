...
export default function InCall() {
  ...
  useEffect(() => {
    if (localVideoRef.current && localVideoTrack) {
      ...
      if (
        currentStream &&
        currentStream.getVideoTracks()[0] === localVideoTrack
      )
        return;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-linear-to-b from-base-200 to-base-100 flex flex-col items-center justify-between overflow-hidden">
      ...
      {isLocalVideoEnabled && (
        <div
          className={`absolute bottom-32 right-6 z-30 w-36 h-48 bg-black rounded-xl overflow-hidden shadow-2xl border border-white/20 transition-all duration-300 ${isRemoteVideoPresent ? '' : 'bottom-40'}`}
        >
          <video
            ref={localVideoRef}
          ...
          {isConnected ? (
            <div className="grid grid-cols-4 gap-4 items-center justify-items-center">
              <div className="flex flex-col items-center gap-2">
                ...
                <span
                  className={`text-xs font-medium ${isRemoteVideoPresent ? 'text-white/80' : 'text-base-content/70'}`}
                >
                  Mute
                </span>
              </div>
