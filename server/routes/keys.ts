...
keys.get('/:number', zValidator('param', paramSchema), (c) => {
  ...
  if (!entry) {
    ...
    db.run(
      'INSERT INTO numbers (number, signing_key, encryption_key, status) VALUES (?, ?, ?, ?)',
      [
        number,
  ...
  if (entry.status === 'waiting') {
    ...
    db.run(
      'UPDATE numbers SET signing_key = ?, encryption_key = ? WHERE number = ?',
      [JSON.stringify(signingKey), JSON.stringify(encryptionKey), number],
    );
